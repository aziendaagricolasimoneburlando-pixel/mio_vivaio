...
# CSS personalizzato per il mood "vivaio"
st.markdown("""
<style>
    ...
    .card {
        background: #fff; border: 1px solid #d9d0b8;
...
def check_password():
    ...
    st.markdown('<div class="main-header"><h1>🌱 Vivaio</h1><div class="sub">Accesso</div></div>', unsafe_allow_html=True)
...
@st.cache_resource
def get_gsheet():
    """Apre il foglio Google usando le credenziali del service account."""
    scopes = [
    ...
    # Assicura che tutti i fogli esistano
...
def read_sheet(tab_name):
    """Legge un foglio e ritorna una lista di dict."""
    sheet = get_gsheet()
...
def write_sheet(tab_name, rows, headers=None):
    """Sovrascrive un foglio con i dati passati."""
    sheet = get_gsheet()
    ws = sheet.worksheet(tab_name)
    ws.clear()
    if not rows:
    ...
    # Normalizza: assicura che tutte le righe abbiano le stesse colonne
    data = [headers]
    for r in rows:
...
# ============================================================
# DATA LAYER — carica e salva lo stato
# ============================================================
def load_state():
    """Carica lo stato dal Google Sheet in session_state."""
    ...
    # Parse varietà (il campo JSON "data" contiene fasi, note, consumi, containers)
    varieta = []
    ...
    for r in contenitori_raw:
        contenitori.append({
            ...
            "volume": float(r.get("volume", 0) or 0),  # in litri
    ...
    # Acquisti (riga = singolo acquisto, legato a materialId)
    acquisti_by_mat = {}
...
def save_varieta():
    ...
    for v in st.session_state.varieta:
        rows.append({
            "id": v["id"],
            "nome": v["nome"],
            "varieta_nome": v["varieta_nome"],
            "categoria": v["categoria"],
            "data_semina": v["data_semina"],
            "quantita": v["quantita"],
            "prezzoVendita": v["prezzoVendita"],
            "data_json": json.dumps({
                "fasi": v.get("fasi", []),
                "note": v.get("note", []),
                "consumi": v.get("consumi", {}),
                "containers": v.get("containers", []),
                "costiExtra": v.get("costiExtra", {}),
            }, ensure_ascii=False),
        })
    headers = ["id", "nome", "varieta_nome", "categoria", "data_semina", "quantita", "prezzoVendita", "data_json"]
    write_sheet("varieta", rows, headers)
...
def save_contenitori():
    ...
    for c in st.session_state.contenitori:
        rows.append({
            ...
            "type": c["type"],
            "volume": c["volume"],
            "materialId": c.get("materialId") or "",
            "unitCost": c.get("unitCost", 0),
        })
    headers = ["id", "name", "type", "volume", "materialId", "unitCost"]
    write_sheet("contenitori", rows, headers)
...
def save_materiali():
    ...
    headers = ["id", "name", "unit", "system"]
    write_sheet("materiali", rows, headers)

    acq_headers = ["materialId", "date", "supplier", "qty", "total"]
    write_sheet("acquisti", acquisti_rows, acq_headers)
...
def calc_container_cost(container):
    ...
    return {
        "terriccio": terriccio_cost,
        "volume": container.get("volume", 0) or 0,
        "container": cont_cost,
        "total": terriccio_cost + cont_cost,
    }
...
def get_fasi_with_dates(variety):
    """Calcola le date delle fasi e quale è corrente/fatta."""
    ...
    for i, f in enumerate(fasi):
        ...
        if d <= today:
            status = "done"
        # La "corrente" è l'ultima fatta che non è l'ultima
        result.append({**f, "date": d, "status": status})
    # Ricalcola: l'ultima "done" diventa "current" se ci sono fasi future dopo
    last_done_idx = -1
...
def next_upcoming_fase(variety):
    """Ritorna la prossima fase non ancora fatta."""
...
def variety_form():
    """Form per creare/modificare una varietà."""
    ...
    with st.form("variety_form"):
        ...
        prezzo = st.number_input("Prezzo vendita unitario (€)", min_value=0.0, step=0.1, value=v["prezzoVendita"] if v else 0.0, format="%.2f")

        preset = st.selectbox(
            "Preset fasi",
            list(FASI_PRESETS.keys()),
        ...
        for m in st.session_state.materiali:
            if m["id"] == "terriccio":
                continue  # il terriccio è calcolato dai contenitori
        ...
        costo_seme = st.number_input("Costo seme per piantina (€)", min_value=0.0, step=0.01, value=seme_default, format="%.3f")
    ...
    if submit:
        ...
        if v:
            ...
            # Se l'utente cambia preset, aggiorniamo le fasi solo se erano quelle di un preset (per sicurezza teniamole sempre dal preset scelto)
            v["fasi"] = [dict(f) for f in fasi_preset]
        else:
            new_v = {
                "id": "v_" + uuid.uuid4().hex[:10],
                "nome": nome.strip(),
                "varieta_nome": varieta_nome.strip(),
                "categoria": categoria,
                "data_semina": data_semina.isoformat(),
                "quantita": int(quantita),
                "prezzoVendita": float(prezzo),
            ...
            st.session_state.varieta.append(new_v)
...
def container_form():
    ...
    with st.form("container_form"):
        ...
        ctype = st.selectbox(
            "Tipo", ["alveolo", "vasetto"],
            index=["alveolo", "vasetto"].index(c["type"]) if c and c.get("type") in ["alveolo", "vasetto"] else 0,
        )
        volume_ml = st.number_input(
            "Volume (ml) — quanto terriccio contiene una cella/vaso",
            min_value=0, step=1,
            value=int((c["volume"] * 1000) if c else 0),
        )
        unit_cost = st.number_input(
            "Costo unitario (€) del singolo contenitore",
...
def nav():
    """Navigazione mobile in basso (duplicata dalla sidebar per dispositivi piccoli)."""
